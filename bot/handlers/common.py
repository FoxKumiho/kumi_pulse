# Путь файла: bot/handlers/common.py
...
import aiogram
from ..modules.no_sql.user_db import register_chat_member, get_user, save_chat, OWNER_BOT_ID, get_all_user_ids, increment_activity_count
...
@router.message(~Command(commands=["start", "warn", "clear_warnings", "clear", "mute", "unmute", "ban", "unban", "kick", "user_status", "mod_logs", "help_moderation", "register_all", "force_register_all"]))
async def message_handler(message: Message):
    """
    Обработчик любых сообщений, кроме команд, для регистрации активных участников чата и подсчета активности.
    """
    chat_id = message.chat.id
    user_id = message.from_user.id
    logger.debug(
        f"Получено сообщение от user_id={user_id}, is_bot={message.from_user.is_bot}, chat_id={chat_id}, message_id={message.message_id}")
    try:
        # Проверяем права бота
        bot_member = await message.bot.get_chat_member(chat_id=chat_id, user_id=message.bot.id)
        if bot_member.status != "administrator" or not bot_member.can_manage_chat:
            logger.warning(
                f"Бот не имеет прав администратора или 'Manage Chat' в chat_id={chat_id}, сообщение от user_id={user_id} проигнорировано")
            return
        await register_chat_member(
            user_id=user_id,
            username=message.from_user.username,
        ...
        if not message.from_user.is_bot:
            ...
            if success:
                ...
                logger.info(
                    f"Счетчик активности увеличен для user_id={user_id} в chat_id={chat_id}, новый счет: {updated_user.get_activity_count(chat_id)}")
            else:
        ...
        logger.info(
            f"Обработано сообщение от user_id={user_id} (is_bot={message.from_user.is_bot}) в chat_id={chat_id}")
    except TelegramBadRequest as e:
        logger.error(
            f"Ошибка при проверке прав бота или регистрации пользователя {user_id} в chat_id={chat_id}: {str(e)}")
    except Exception as e:
...
async def register_all_chat_members(chat_id: int, bot):
    ...
    try:
        ...
        for admin in admins:
            ...
            logger.info(
                f"Зарегистрирован администратор {admin.user.id} (is_bot={admin.user.is_bot}) для chat_id={chat_id}")

        # Регистрируем известных пользователей
        ...
        for user_id in all_user_ids:
            try:
                ...
                if member.status not in ("left", "kicked"):
                    ...
                    logger.info(
                        f"Зарегистрирован участник {member.user.id} (is_bot={member.user.is_bot}) для chat_id={chat_id} через get_chat_member")
            except TelegramBadRequest as e:
...
async def force_register_all_handler(message: Message):
    ...
    try:
        ...
        if user.role_level < 6 and user.user_id != OWNER_BOT_ID and chat_id not in user.server_owner_chat_ids:
            await message.answer("У вас нет прав для этой команды! Требуется роль Владелец сервера или выше.")
            logger.warning(
                f"Пользователь {user_id} попытался выполнить /force_register_all без прав, chat_id={chat_id}")
            return
        await save_chat(chat_id, message.chat.title)
        ...
        # Регистрируем администраторов и участников
        await register_all_chat_members(chat_id, message.bot)
        ...
        for user_id in all_user_ids:
            try:
                ...
                if member.status not in ("left", "kicked"):
                    ...
                    registered_count += 1
                    logger.info(
                        f"Зарегистрирован участник {member.user.id} (is_bot={member.user.is_bot}) для chat_id={chat_id} через /force_register_all")
        ...
        logger.info(
            f"Команда /force_register_all выполнена для chat_id={chat_id} пользователем {user_id}, зарегистрировано: {registered_count}")
    except Exception as e:
